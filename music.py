...
def kugou(name):
    ...
    all_music = one_dic['data']['lists']
    music = []
    if len(all_music) >= 5:
        for one_music in all_music[:5]:
            data = {}
            data['歌曲'] = one_music['FileName'].replace('<', '').replace('em', '').replace('/', '').replace('>', '')
            data['歌手'] = one_music['SingerName']
    ...
    for one_music in music:
        ...
        response = requests.get(url=url, headers=headers).content.decode('utf-8')
        one_music['标准链接'] = json.loads(response, strict=False)['data']['play_url']
        ...
        if one_music['hq_hash']:
            ...
            response = requests.get(url=url, headers=headers).content.decode('utf-8')
            one_music['高清链接'] = json.loads(response)['data']['play_url']
            if not one_music['高清链接']:
                del one_music['高清链接']
